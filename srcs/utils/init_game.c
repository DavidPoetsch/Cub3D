...
/*   Updated: 2025/04/17 13:54:25 by lstefane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int import_sprite_textures(void *mlx, t_map *map)
{
	...
	res = open_img(mlx, &map->sprite[0].texs[0], "./test/textures/soldier.xpm");
	if (res == SUCCESS)
		res = open_img(mlx, &map->sprite[1].texs[0], "./test/anims/torch/torch00.xpm");
	if (res == SUCCESS)
		res = open_img(mlx, &map->sprite[1].texs[1], "./test/anims/torch/torch01.xpm");
	if (res == SUCCESS)
		res = open_img(mlx, &map->sprite[1].texs[2], "./test/anims/torch/torch02.xpm");
	if (res == SUCCESS)
		res = open_img(mlx, &map->sprite[1].texs[3], "./test/anims/torch/torch01.xpm");
	return (res);
}
...
int setup_sprites(t_game *game)
{
	...
	game->map.sprite[0].tex_count = 1;
	game->map.sprite[0].texs = ft_calloc(game->map.sprite[0].tex_count, sizeof(t_sprite));
	if (!game->map.sprite[0].texs)
		return (result_failed("ft_calloc", __func__, __FILE__));
	game->map.sprite[0].pos.x = 2.0;
	game->map.sprite[0].pos.y = 20.5;
	game->map.sprite[0].hidden = false;
	game->map.sprite[0].type = OBJECT;
	game->map.sprite[0].size_adjust = 1;
	game->map.sprite[0].update_t = 0;
	game->map.sprite[0].move = 100;
	game->map.sprite[0].t_id = 0;
	//--------------------------------------------------------------------------------
	game->map.sprite[1].tex_count = 4;
	game->map.sprite[1].texs = ft_calloc(game->map.sprite[0].tex_count, sizeof(t_sprite));
	if (!game->map.sprite[1].texs)
		return (result_failed("ft_calloc", __func__, __FILE__));
	game->map.sprite[1].pos.x = 1.05;
	game->map.sprite[1].pos.y = 20.5;
	game->map.sprite[1].hidden = false;
	game->map.sprite[1].type = OBJECT;
	game->map.sprite[1].size_adjust = 2;
	game->map.sprite[1].update_t = 500;
	game->map.sprite[1].move = -100;
	game->map.sprite[1].t_id = 0;
